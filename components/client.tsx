...
export default function Client({ client, selected, setSelected }: IProps): JSX.Element {
    ...
    return (
    <div onClick={() => setSelected(client)}  className={`flex gap-5 justify-between cursor-pointer border border-slate-200 shadow rounded-md p-4 px-5 w-full dark:bg-slate-700 dark:border-transparent ${selected?.id == id ? 'bg-slate-100 dark:bg-slate-800' : ''}`} key={client.id}>
